...
class ClaudeProvider(LLMProvider):
    ...
    def __init__(
        ...
        Args:
            ...
            api_key: API 키 (미지정 시 환경변수 MY_ANTHROPIC_KEY 사용)
            max_tokens: 최대 토큰 수
        ...
        Note:
            이 프로젝트는 MY_ANTHROPIC_KEY 환경변수만 사용합니다.
            Claude Code 등 외부 도구와 키 네임스페이스가 섞이지 않도록
            ANTHROPIC_API_KEY는 의도적으로 읽지 않습니다.
        """
        self.model = model
        # API 키 결정: 인자 > MY_ANTHROPIC_KEY (ANTHROPIC_API_KEY는 읽지 않음)
        ...
        if not self.api_key:
            raise ExtractionError(
                ...
                "MY_ANTHROPIC_KEY 환경변수를 설정하세요. "
                "(이 프로젝트는 ANTHROPIC_API_KEY를 사용하지 않습니다)",
            )
